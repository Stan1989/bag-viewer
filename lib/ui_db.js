...
module.exports.initQueryFromUIDB = function(dbPath, nodes) {
    var db = new sqlite3.Database(dbPath);
    db.serialize(function () {
        db.each("SELECT id, name, sql, count_sql FROM query",
            function (err, row) {
                nodes.push({name: row.name, sql: row.sql, count_sql: row.count_sql})
            },
            function () {
                db.close();
            }
...
module.exports.initListDisplayFromUIDB = function(dbPath, nodes) {
    var db = new sqlite3.Database(dbPath);
    db.serialize(function () {
        db.each("SELECT list.id AS id, record_count_per_page, name, displayname, displayname_en, width, data_type FROM list left join ui_list_column on list.id = ui_list_column.ui_list_id",
            function (err, row) {
                //nodes.push({name: row.name, displayname: row.displayname, displayname_en: row.displayname_en, width : row.width, data_type : row.data_type})
            },
            function () {
                db.close();
            }
...
module.exports.loadUITree = function (dbPath, callback) {
    ...
    db.serialize(function () {
        db.each("SELECT * FROM ui_tree",
            function (err, row) {
                treeNodes.push({ id: row.id, name: row.display_name, parent: row.parent_id, query_id : row.query_id, record_id : row.record_id, param: row.param });
            },
            function () {
